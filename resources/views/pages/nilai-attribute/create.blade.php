        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="w-full lg:w-1/2 mx-auto xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
                <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                    <h2 class="font-semibold text-slate-800 dark:text-slate-100">Form Tambah Nilai ({{$attribute->kode}}) {{$attribute->nama}}</h2>
                </header>
                <div class="p-3">
                    <form action="{{route('nilai-attribute.save', $attribute->id)}}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <x-jet-label for="value" value="{{ __('Value Attribute') }}" />
                            <x-jet-input id="value" type="text" name="value" :value="old('value')" required autofocus />                
                        </div>
                        <div class="mb-4">
                            <x-jet-label for="nilai" value="{{ __('Nilai Attribute') }}" />
                            <x-jet-input id="nilai" step="0.1" type="number" name="nilai" :value="old('nilai')" required />                
                        </div>
